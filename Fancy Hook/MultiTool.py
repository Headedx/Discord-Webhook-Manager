...
black = "\033[1;30m"
titletext = " [-- FANCYHOOK --] Made by xheaded"
red = "\033[1;31m"
green = "\033[1;32m"
yellow = "\033[1;33m"
blue = "\033[1;34m"
...
cyan = "\033[1;36m"
white = "\033[1;37m"
invalidurl = f"{red}[! FANCYHOOK !]{white} Invalid url!"
# test = "" test webhook, dont forget to remove :3

socials = {
    "github": {"link": "https://github.com/Headedx"},
...
██║░░░░░██║░░██║██║░╚███║╚█████╔╝░░░██║░░░  ░░░██║░░░╚█████╔╝╚█████╔╝███████╗
╚═╝░░░░░╚═╝░░╚═╝╚═╝░░╚══╝░╚════╝░░░░╚═╝░░░  ░░░╚═╝░░░░╚════╝░░╚════╝░╚══════╝            
    >> [Webhook Multitool developed by @xheaded]
"""
...
def changepfp(url):
    input(f"{yellow}[? FANCYHOOK ?]{white} Press enter to select file or skip this to input the path/url")
    image_path = fd.askopenfilename(filetypes=[("Profile Pictures", "*.png;*.jpg;*.jpeg")])
    if image_path is None or image_path == "":
        clear()
        image_path = input(f"{yellow}[? FANCYHOOK ?]{white} Path/URL to image: ")
    ...
    try:
        ...
        response.raise_for_status()
        print(f"{green}[+ FANCYHOOK +]{white} Profile picture changed successfully.")
    except FileNotFoundError:
        print(f"{red}[! FANCYHOOK !] File not found. Please provide a valid file path or image url.")
    except requests.exceptions.HTTPError as errh:
        print(f"{red}[! FANCYHOOK !] HTTP Error: {errh}")
    except requests.exceptions.ConnectionError as errc:
        print(f"{red}[! FANCYHOOK !] Error Connecting: {errc}")
    except requests.exceptions.Timeout as errt:
        print(f"{red}[! FANCYHOOK !] Timeout Error: {errt}")
    except requests.exceptions.RequestException as err:
        print(f"{red}[! FANCYHOOK !] Request Exception: {err}")

def deletehook(url):
    print(f"{cyan}[+ FANCYHOOK +]{white} Trying to delete webhook...")
    try:
        response = requests.delete(url)
        response.raise_for_status()  # Raises HTTPError for bad responses
        print(f"{green}[+ FANCYHOOK +]{white} Webhook deleted successfully.")
    except requests.exceptions.HTTPError as errh:
        print(f"{red}[! FANCYHOOK !] HTTP Error: {errh}")
...
def sendmessage(url):
    msg = input(f"{yellow}[? FANCYHOOK ?]{white} Message: ")
    try:
        response = requests.post(url, json={"content": msg})
        response.raise_for_status()
        print(f"{green}[+ FANCYHOOK +]{white} Message sent successfully.")
...
def renamehook(url):
    name = input(f"{yellow}[? FANCYHOOK ?]{white} Webhook Name: ")
    print(f"{cyan}[+ FANCYHOOK +]{white} Trying to change username...")
    try:
        response = requests.patch(url, json={"name": name})
        response.raise_for_status()
        print(f"{green}[+ FANCYHOOK +]{white} Webhook name changed successfully.")
...
def spamhook(url):
    print(f"{cyan}[+ FANCYHOOK +]{white} Trying to spam webhook...")
    msg = input(f"{yellow}[? FANCYHOOK ?]{white} Spam Text: ")
    timeout = float(input(f"{yellow}[? FANCYHOOK ?]{white} Timeout (to avoid api-ratelimit): "))
    try:
        print(f"{red}[! FANCYHOOK !] Spam has started, Relaunch the tool to stop spam and use it again.")
        while True:
            response = requests.post(url, json={"content": msg})
            response.raise_for_status()
            print(f"{green}[+ FANCYHOOK +]{white} Sent message")
            time.sleep(timeout)
    except requests.exceptions.HTTPError as errh:
        print(f"{red}[! FANCYHOOK !] HTTP Error: {errh}")
...
webhook = {}
os.system("title github.com/Headedx")
while True:
    clear()
    printascii()
    while True:
    ...
    while True:
        ...
        print(f"\n\n\n{green}[+ FANCYHOOK +]{white} Logged into webhook: {webhook_name}")
        ...
        if ch == 1:
            ...
            print(f"{cyan}[+ FANCYHOOK +]{white} Source code can be found here:")
